...
class _SavedLoansScreenState extends State<SavedLoansScreen> {
  ...
  Future<void> _load() async {
    final data = await DatabaseService.getLoanConfigs();
        ...
        actions: [
          if (_selectMode && _selected.length >= 2)
            TextButton.icon(
              ...
              onPressed: () {
                final loans = _loans.where((l) => _selected.contains(l.id)).toList();
                Navigator.push(
                  context,
                  MaterialPageRoute(builder: (_) => LoanCompareScreen(loans: loans)),
                );
              },
            ),
          if (_selectMode)
